...
public class Boss extends Monstres{
    ...
    private Monstres cible(){
        ...
        for(Monstres m : toursEnRange){
            if(hypot@@hénus(distance(m.position.getX(), this.position.getX()),distance(m.position.getY(), this.position.getY()))<hypothénus(distance(temp.position.getX(), this.position.getX()),distance(temp.position.getY(), this.position.getY()))){
            temp = m;
            }
        }
        return temp;
