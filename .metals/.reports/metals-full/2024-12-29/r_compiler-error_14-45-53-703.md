...
public class Boss extends Monstres{
    ...
    private Tours cible(){
        ...
        for(Tours t : toursEnRange){
            if(hypothénus(d@@istance(t.position.getX(), this.position.getX()),distance(t.position.getY(), this.position.getY()))<hypothénus(distance(temp.position.getX(), this.position.getX()),distance(temp.position.getY(), this.position.getY()))){
            temp = t;
            }
        }
        return temp;
